package mk.ukim.finki.np.lab4.task3;
...
class Queue<T> {
    ...
    public void enqueue(T element) {
        Node<T> newElem = new Node<T>(element, null);
        if (this.count == 0){
            this.start = this.end = newElem;
        } else {
            this.end.setNext(newElem);
            this.end = newElem;
        }
        this.count++;
    }
    ...
    public T dequeue() throws EmptyQueueException {
        if(isEmpty()) throw new EmptyQueueException();
        T element = this.start.getElement();
