package mk.ukim.finki.np.lab7.task1;

import java.util.*;


class Scheduler<T> {
    private TreeMap<Date, T> schedulerMap;
    ...
    public void add(Date date, T element) {
        schedulerMap.put(date, element);
    }

    public boolean remove(Date date) {
        return schedulerMap.remove(date) != null;
    }

    public T next() {
        return schedulerMap.ceilingEntry(new Date()).getValue(); // new Date == now
    }
    ...
    public ArrayList<T> getAll(Date begin, Date end) {
        ...
        list.addAll(schedulerMap.subMap(begin, end).values());
        return list;
    }
