package mk.ukim.finki.np.lab1.task3;
...
final class IntegerArray {
    ...
    public double average() {
        OptionalDouble optionalDouble = Arrays.stream(niza).average();
        return optionalDouble.isPresent() ? optionalDouble.getAsDouble() : 0d;
    }

    public IntegerArray getSorted() {
        int[] copyArr = Arrays.copyOf(this.niza, this.niza.length);
        Arrays.sort(copyArr);
        return new IntegerArray(copyArr);
    }

    public IntegerArray concat(IntegerArray ia) {
        int[] newArray = Arrays.copyOf(this.niza, ia.length() + this.niza.length);
        int k = 0;
        for (int i = this.niza.length; i < newArray.length; i++) {
            newArray[i] = ia.getElementAt(k);
            k++;
        }
